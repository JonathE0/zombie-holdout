...
test('Iron Golem: shrunk to fit under a level-1 floor, and attacks a floor/ramp pinning it down', () => {
  ...
  const room = started(new HoldoutRoom('Z6', {}));
  room.startWave(7); room.director.queue = [];
  for (const z of [...room.zombies.values()]) room.removeZombie(z);
  const g = room.spawnZombie('golem', 'N', '');
  g.pos = [0, 0, -10];
  ...
  let hit = false;
  for (let i = 0; i < 80 && !hit; i++) { g.stuck = 1; updateZombies(room, 0.05, T += 60); hit = floor.hp < hpBefore; }
  assert.ok(hit, 'a breaker stuck under a floor attacks it, not just walls in front of it');
});
...
test('Wave 10: two Brood Titans arrive at once (from different lanes), each with two riders leaping immediately, minions every ~8s, and permanent Sniper Riders that stay mounted and immune until their own Titan dies', () => {
  const room = started(new HoldoutRoom('Z4', {}));
  const a = join(room, 'A'), p = a.player;
  p.st.p = [40, 0, 40];
  room.startWave(10); room.director.queue = [];
  for (const z of [...room.zombies.values()]) room.removeZombie(z);
  advance(room, 5400); // 'coming' warning (5s), then they spawn
  ...
  assert.equal(riders.filter(r => !r.mount).length, 4, 'two normal riders per Titan leap down the moment it arrives');
  ...
  assert.ok(after >= before + 4, 'both Titans drop 2 fresh runners/stalkers off their backs every ~8s');
  ...
  advance(room, 100);
  const [deadSnipers, aliveSnipers] = [snipers.filter(r => !r.mount), snipers.filter(r => r.mount)];
  assert.ok(deadSnipers.length === 2 && aliveSnipers.length === 2, 'only the fallen Titan\'s Sniper Riders fall off');
  room.killZombie(titans[1], p, 'rocket');
  advance(room, 100);
  assert.ok(snipers.every(r => !r.mount), 'fall off, stunned, once their Titan dies');
