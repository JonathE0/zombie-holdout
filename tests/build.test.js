...
test('floors sit flush with their level line: a standing player clears a ground-level door, walls and floors keep their integrity roles', () => {
  ...
  const floor = pieceBox({ kind: 'floor', ...tile(0, 0), l: 0, mat: 'wood' });
  const mask = (1 << 0) | (1 << 3); // bottom two tiles of column 0: a one-tile door
  const wall = { id: 9, kind: 'wall', ...tile(0, -4), l: 0, o: 0, mat: 'wood', mask };
  ...
  const wSupport = { id: 1, kind: 'wall', ...tile(-4, -8), l: 0, o: 0, mat: 'wood' };
  const wBox = Object.assign(pieceBox(wSupport), { sid: 1 });
  const fUp = { id: 2, kind: 'floor', ...tile(-4, -8), l: 1, mat: 'wood' };
  ...
  const fGround = { id: 3, kind: 'floor', ...tile(8, 8), l: 0, mat: 'wood' };
